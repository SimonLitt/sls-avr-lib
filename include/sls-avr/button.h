...
typedef struct {
	uint8_t _counter;	/**< Clamped or released state cycle counter. \remark Internal use only! */
	#ifndef __BTN_ALLOW_FAST_SCAN
	uint8_t _up_counter; /**< Counter of cycles of released state after pressing. \remark Internal use only! */
	#endif // __BTN_ALLOW_FAST_SCAN
	byte_t state;		/**< Stages and states flags of click processing */
} btn_info_struct;
...
#ifndef __BTN_ALLOW_FAST_SCAN
#	define BTN_INFO_STRUCT_DEFAULT {._counter = 0, ._up_counter = 0, .state = BTN_INFO_STATE_DEFAULT} /**< Default button information structure #btn_info_struct */
#else
...
#endif // BTN_ALLOW_LONG
//** \remark As 8bit, then no needs in ATOMIC_BLOCK */
